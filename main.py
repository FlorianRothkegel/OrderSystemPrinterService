...
class PrinterServiceApp:
    def __init__(self, *, instance_name: str | None = None, config_path: str | None = None) -> None:
        self.instance_name = (instance_name or "").strip()
        self.config_path = resolve_config_path(instance_name=self.instance_name or None, config_path=config_path)
        self.root = Tk()
        title_suffix = f" - {self.instance_name}" if self.instance_name else ""
        self.root.title(f"OrderSystem Printer Service{title_suffix}")
        ...
        if self.instance_name and loaded.agent_name == default_agent_name:
            loaded.agent_name = f"{default_agent_name}-{self.instance_name}"
        ...
        self.oidc_token_url = StringVar(value=loaded.oidc_token_url)
        self.oidc_client_id = StringVar(value=loaded.oidc_client_id)
        self.oidc_client_secret = StringVar(value=loaded.oidc_client_secret)
        ...
        self.epson_epos_host = StringVar(value=loaded.epson_epos_host or _extract_epos_host_from_url(loaded.epson_epos_url))
    ...
    def _build_ui(self) -> None:
        ...
        common_fields = [
            ...
            ("Keycloak Token URL", self.oidc_token_url, ""),
            ("Keycloak Client ID", self.oidc_client_id, ""),
            ("Keycloak Client Secret", self.oidc_client_secret, "*"),
        ...
        ttk.Combobox(
            ...
            state="readonly",
            values=("preview", "file", "command", "escpos-network", "epson-epos"),
        ...
        self._build_epson_epos_settings()
        ...
        ttk.Label(frame, text=f"Config: {Path(self.config_path)}").grid(row=7, column=0, sticky="w", pady=(8, 0))
    ...
    def _build_epson_epos_settings(self) -> None:
        frame = ttk.Frame(self.mode_container)
        frame.columnconfigure(1, weight=1)
        self._add_labeled_entry(frame, 0, "Epson Printer IP / Host", self.epson_epos_host)
        frame.grid(row=0, column=0, sticky="ew")
        self.mode_frames["epson-epos"] = frame
    ...
    def _set_running_ui(self, running: bool) -> None:
        for button in (self.save_button, self.start_button, self.stop_button, self.test_button, self.health_button):
            button.grid_forget()

        if running:
            self.settings_frame.grid_remove()
            self.stop_button.grid(row=0, column=0, columnspan=4, sticky="ew", padx=4)
        ...
        self.test_button.grid(row=0, column=2, sticky="ew", padx=4)
        self.health_button.grid(row=0, column=3, sticky="ew", padx=4)
        self._refresh_mode_fields()
    ...
    def _current_config(self) -> AppConfig:
        ...
        return AppConfig(
            ...
            oidc_token_url=self.oidc_token_url.get().strip(),
            oidc_client_id=self.oidc_client_id.get().strip(),
            oidc_client_secret=self.oidc_client_secret.get().strip(),
            ...
            epson_epos_host=self.epson_epos_host.get().strip(),
            epson_epos_url="",
        )
    ...
    def _save_clicked(self) -> None:
        ...
        self._log(f"Saved config to {Path(self.config_path)}")
    ...
    def _test_print_clicked(self) -> None:
        ...
        try:
            printer = TicketPrinter(config, log_callback=self._log)
    ...
    def _run_health_check(self, config: AppConfig) -> None:
        ...
        printer_status = check_printer_health(config, log_callback=self._log)
        if not self.closed:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="OrderSystem Printer Service")
    ...
    PrinterServiceApp(instance_name=args.instance_name, config_path=args.config_path).run()
