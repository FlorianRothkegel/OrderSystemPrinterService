...
```bash
chmod +x printerService/build-linux-arm32.sh
./printerService/build-linux-arm32.sh
```

Build Raspberry Pi OS 32-bit from a 64-bit Linux machine or 64-bit Pi using Docker:

```bash
chmod +x printerService/build-linux-arm32-docker.sh
./printerService/build-linux-arm32-docker.sh
```
...
```bash
chmod +x printerService/build-linux-arm32-headless.sh
./printerService/build-linux-arm32-headless.sh
```

Build Raspberry Pi OS 32-bit headless from a 64-bit Linux machine or 64-bit Pi using Docker:

```bash
chmod +x printerService/build-linux-arm32-headless-docker.sh
./printerService/build-linux-arm32-headless-docker.sh
```
...
- Linux amd64: `OrderSystemPrinterService-linux-amd64`
- Linux arm64: `OrderSystemPrinterService-linux-arm64`
- Linux arm32 / Raspberry Pi OS 32-bit: `OrderSystemPrinterService-linux-arm32`
- Windows amd64: `OrderSystemPrinterService-windows-amd64.exe`
- and the matching `Headless` variants for each target
...
- the `build-linux-arm32*.sh` scripts are for 32-bit Raspberry Pi OS / `armv7l` userspace only
- the `build-linux-arm32*-docker.sh` scripts run the 32-bit build inside an `arm32v7` Docker container, which works on a 64-bit Pi agent and can work on a normal Linux machine when Docker multiarch / binfmt support is available
