...
def check_backend_health(config: AppConfig, timeout: float = 3.0) -> HealthStatus:
    ...
    if not config.access_token.strip():
        return HealthStatus(False, "Connected, printer access token missing")
    if not config.printer_secret.strip():
        return HealthStatus(False, "Connected, printer secret missing")
    ...
    auth_req = request.Request(
        ...
        headers={
            "Accept": "application/json",
            "User-Agent": "OrderSystemPrinterService/1.0",
            "X-Access-Token": config.access_token.strip(),
            "X-Printer-Secret": config.printer_secret.strip(),
        },
        method="GET",
    )
    try:
        ...
        if status_code == 401:
            ...
            if "printer service secret" in message:
                return HealthStatus(False, "Connected, printer secret invalid")
            if "printer access token" in message:
                return HealthStatus(False, "Connected, printer token invalid or rotated")
        ...
        try:
            ...
            if not exc.retryable:
                ...
                if exc.status_code == 401:
                    if "printer access token" in lowered:
                        log_callback(
                            f"Backend authorization failed: {exc}. Update the printer token and retrying in {delay}s."
                        )
                    elif "printer service secret" in lowered:
                        log_callback(
                            f"Backend authorization failed: {exc}. Check the printer secret and retrying in {delay}s."
                        )
                    else:
                        log_callback(
                            f"Backend authorization failed: {exc}. Check the printer service credentials and retrying in {delay}s."
                        )
                else:
