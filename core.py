...
MAX_PRINT_RETRY_DELAY_SECONDS = 4
MAX_EPSON_JOB_TIMEOUT_MS = 3000
...
def _extract_epos_host_from_url(raw_url: str) -> str:
...
def _epos_endpoint_url(config: AppConfig) -> str:
    host = config.epson_epos_host.strip() or _extract_epos_host_from_url(config.epson_epos_url)
    if not host:
        return ""
    if "://" in host:
        parsed_host = parse.urlsplit(host)
    ...
    return parse.urlunsplit(
        (
            ...
            parse.urlencode(
                {
                    "devid": "local_printer",
                    "timeout": str(MAX_EPSON_JOB_TIMEOUT_MS),
                }
            ),
            "",
        )
...
def load_config(config_path: str | Path | None = None) -> AppConfig:
    ...
    try:
        ...
        if not normalized.get("epson_epos_host") and normalized.get("epson_epos_url"):
            normalized["epson_epos_host"] = _extract_epos_host_from_url(str(normalized["epson_epos_url"]))
...
EPOS_PRINT_XML_NS = "http://www.epson-pos.com/schemas/2011/03/epos-print"
SOAP_ENV_NS = "http://schemas.xmlsoap.org/soap/envelope/"
EPOS_ERROR_MESSAGES = {
    "EPTR_REC_EMPTY": "Paper out",
    "EPTR_COVER_OPEN": "Cover open",
    "EPTR_CUTTER": "Autocutter error",
    "EPTR_MECHANICAL": "Mechanical error",
    "EPTR_AUTOMATICAL": "Automatic recovery error",
    "EPTR_BATTERY_LOW": "Battery low",
    "EPTR_UNRECOVERABLE": "Unrecoverable printer error",
    "0x02000000": "Cannot print on paper",
    "0x80000000": "Printer spooler stopped",
}
EPOS_NON_ERROR_CODES = {"SUCCESS", "Printing", "JobSpooling"}


def _epos_print_root(*, force: bool = False) -> ET.Element:
    attributes = {"xmlns": EPOS_PRINT_XML_NS}
    if force:
        attributes["force"] = "true"
    return ET.Element("epos-print", attributes)


def _epos_add_text(root: ET.Element, text: str = "", **attrs: object) -> ET.Element:
    normalized_attrs = {key: str(value) for key, value in attrs.items() if value is not None}
    element = ET.SubElement(root, "text", normalized_attrs)
    element.text = text
    return element


def _epos_wrap_soap_envelope(epos_print: ET.Element) -> bytes:
    envelope = ET.Element(f"{{{SOAP_ENV_NS}}}Envelope")
    body = ET.SubElement(envelope, f"{{{SOAP_ENV_NS}}}Body")
    body.append(epos_print)
    return ET.tostring(envelope, encoding="utf-8", xml_declaration=True)


def _epos_wrap_escpos_command(command_bytes: bytes) -> bytes:
    root = _epos_print_root()
    command = ET.SubElement(root, "command")
    command.text = command_bytes.hex()
    return _epos_wrap_soap_envelope(root)


def _epos_blank_health_request() -> bytes:
    return _epos_wrap_soap_envelope(_epos_print_root())


def _epos_parse_response(body: str) -> tuple[bool, str, str]:
    try:
        root = ET.fromstring(body)
    except ET.ParseError as exc:
        raise RuntimeError("Invalid Epson ePOS XML response") from exc

    fault_message = ""
    for candidate in root.iter():
        local_name = candidate.tag.rsplit("}", 1)[-1] if "}" in candidate.tag else candidate.tag
        if local_name == "faultstring":
            fault_message = (candidate.text or "").strip()
            break

    response_element: ET.Element | None = None
    for candidate in root.iter():
        local_name = candidate.tag.rsplit("}", 1)[-1] if "}" in candidate.tag else candidate.tag
        if local_name == "response":
            response_element = candidate
            break

    target = response_element if response_element is not None else root
    success_raw = str(target.attrib.get("success", "")).strip().lower()
    code = str(target.attrib.get("code", "")).strip()
    status = str(target.attrib.get("status", "")).strip()
    if not code and fault_message:
        code = fault_message
    if not code and response_element is None:
        code = "EPOS_RESPONSE_MISSING"
    success = success_raw in {"true", "1"}
    return success, code, status


def _epos_status_indicates_completion(status: str) -> bool:
    ...
    try:
        ...
        return False
    return (value & 0x00000002) == 0x00000002


def _epos_response_is_success(success: bool, code: str, status: str) -> bool:
    normalized_code = code.strip()
    if success:
        return True
    if normalized_code in EPOS_NON_ERROR_CODES:
        return True
    if _epos_status_indicates_completion(status):
        return True
    return False


def _epos_message_from_code(code: str, status: str = "") -> str:
    normalized = code.strip()
    if normalized == "EPOS_RESPONSE_MISSING":
        return "ePOS response missing"
    if normalized in EPOS_ERROR_MESSAGES:
        return EPOS_ERROR_MESSAGES[normalized]
    if normalized:
        return f"ePOS error {normalized}"
    if status:
        return f"ePOS status {status}"
    return "ePOS print failed"
...
def _epos_post(
    ...
    *,
    ...
    query = parse.parse_qsl(parsed_url.query, keep_blank_values=True)
    ...
    for key, value in query:
        if key.lower() == "timeout":
            saw_timeout = True
            try:
                timeout_ms = min(MAX_EPSON_JOB_TIMEOUT_MS, max(1, int(value)))
            except ValueError:
                timeout_ms = MAX_EPSON_JOB_TIMEOUT_MS
            updated_query.append((key, str(timeout_ms)))
        else:
            updated_query.append((key, value))
    if not saw_timeout:
        updated_query.append(("timeout", str(MAX_EPSON_JOB_TIMEOUT_MS)))
    normalized_url = parse.urlunsplit(
        (
            parsed_url.scheme,
            parsed_url.netloc,
            parsed_url.path,
            parse.urlencode(updated_query),
            parsed_url.fragment,
        )
    )
    req = request.Request(
    ...
    try:
        ...
        raise RuntimeError(f"ePOS endpoint unreachable: {exc.reason}") from exc
    except TimeoutError as exc:
        raise RuntimeError("ePOS endpoint timed out") from exc

    if not 200 <= status_code < 300:
        raise RuntimeError(_summarize_http_error(status_code, body, content_type))

    if log_callback is not None:
        compact_body = " ".join(body.split())
        log_callback(f"Epson ePOS raw response: {compact_body[:500]}")

    success, code, status = _epos_parse_response(body)
    return _epos_response_is_success(success, code, status), code, status


def _epos_health(config: AppConfig, timeout: float = 5.0) -> HealthStatus:
    url = _epos_endpoint_url(config)
    if not url:
        return HealthStatus(False, "Epson ePOS host missing")
    try:
        success, code, status = _epos_post(url, _epos_blank_health_request(), timeout)
    except Exception as exc:
        return HealthStatus(False, str(exc))
    if success:
        return HealthStatus(True, "ePOS ready")
    if code in {"EPOS_RESPONSE_MISSING"} or code.lower().startswith("server was unable") or code.lower().startswith("soap"):
        return HealthStatus(True, "ePOS endpoint reachable")
    return HealthStatus(False, _epos_message_from_code(code, status))


def _escpos_network_printer_status(
    config: AppConfig,
    timeout: float,
    *,
    log_callback: Callable[[str], None] | None = None,
    ...
    port = int(config.escpos_port)

    try:
        with socket.create_connection((host, port), timeout=timeout):
            pass
        return HealthStatus(True, f"Connected to {host}:{port}")
    except (socket.timeout, TimeoutError):
        return HealthStatus(False, f"Printer unreachable: timed out")
    except OSError as exc:
        return HealthStatus(False, f"Printer unreachable: {exc}")
    except Exception as exc:
        return HealthStatus(False, f"Printer status error: {exc}")


def check_printer_health(
    config: AppConfig,
    timeout: float = 3.0,
    *,
    log_callback: Callable[[str], None] | None = None,
    ...
    if mode == "escpos-network":
        return _escpos_network_printer_status(config, timeout, log_callback=log_callback)

    if mode == "epson-epos":
        return _epos_health(config, timeout=max(timeout, 5.0))
...
class BackendClient:
    ...
    def reprint_job(self, job_id: str, agent_name: str) -> dict:
        result = self._post(f"/print-service/jobs/{parse.quote(job_id)}/reprint", {"agent_name": agent_name.strip()})
        if not isinstance(result, dict):
            raise BackendRequestError("Reprint job response was invalid", retryable=False)
        return result
...
class TicketPrinter:
    ...
    def _render_escpos_bytes(self, job: dict, job_id: str) -> bytes:
        ...
        if self.config.escpos_cut_paper:
            ...
            parts.append(self._cut_with_feed(30))
    ...
    def print_job(self, job: dict, ticket_text: str, job_id: str) -> str:
        ...
        if self.config.printer_mode == "epson-epos":
            url = _epos_endpoint_url(self.config)
            if not url:
                raise RuntimeError("Epson ePOS host is empty")
            escpos_payload = self._render_escpos_bytes(job, job_id)
            success, code, status = _epos_post(
                url,
                _epos_wrap_escpos_command(escpos_payload),
                30.0,
                log_callback=self.log_callback,
            )
            if self.log_callback is not None:
                self.log_callback(
                    f"Epson ePOS response success={success} code={code or 'none'} status={status or 'none'}"
                )
            if not success:
                raise RuntimeError(_epos_message_from_code(code, status))
            return f"Printed via Epson ePOS to {url}"
...
def validate_required_config(config: AppConfig) -> list[str]:
    ...
    if config.printer_mode == "epson-epos" and not _epos_endpoint_url(config):
        missing.append("Epson ePOS Host")
    return missing
...
def _next_retry_delay(attempt: int, base_delay: int, *, max_delay: int = MAX_RETRY_DELAY_SECONDS) -> int:
    normalized_base = max(1, int(base_delay))
    return min(max(1, int(max_delay)), normalized_base * (2 ** max(0, attempt)))
...
def _is_printer_outage(config: AppConfig, exc: Exception, health_status: HealthStatus | None = None) -> bool:
    ...
    if mode == "epson-epos":
        ...
        if health_status is not None and not health_status.ok:
            health_message = health_status.message.lower()
            return any(fragment in health_message for fragment in generic_transient_fragments)
        return False
...
def _is_paper_out_error(exc: Exception) -> bool:
    message = str(exc).lower()
    return "paper out" in message or "papier leer" in message or "eptr_rec_empty" in message
    ...
    while not stop_requested():
        try:
            ...
            delay = _next_retry_delay(
                ...
                max_delay=MAX_PRINT_RETRY_DELAY_SECONDS,
            )
            if attempt == 0:
...
def run_worker_loop(
    ...
    printer = TicketPrinter(config, log_callback=log_callback)
    backend_retry_attempt = 0
    backend_unavailable = False
    blocked_reprint_source_job_id: str | None = None
    printer_recovery_attempt = 0

    while not stop_requested():
        job_id: str | None = None
        try:
            if blocked_reprint_source_job_id is not None:
                health_status = check_printer_health(config)
                if not health_status.ok:
                    delay = _next_retry_delay(
                        printer_recovery_attempt,
                        config.poll_interval_seconds,
                        max_delay=MAX_PRINT_RETRY_DELAY_SECONDS,
                    )
                    if printer_recovery_attempt == 0:
                        log_callback(
                            f"Waiting for printer recovery before reprinting {blocked_reprint_source_job_id}: {health_status.message}. Retrying in {delay}s."
                        )
                    else:
                        log_callback(
                            f"Printer still not ready to reprint {blocked_reprint_source_job_id}: {health_status.message}. Retrying in {delay}s."
                        )
                    printer_recovery_attempt += 1
                    _sleep_with_stop(delay, stop_requested)
                    continue

                if printer_recovery_attempt > 0:
                    log_callback(f"Printer recovered. Recreating failed job {blocked_reprint_source_job_id} for reprint.")
                job = client.reprint_job(blocked_reprint_source_job_id, config.agent_name)
                blocked_reprint_source_job_id = None
                printer_recovery_attempt = 0
            else:
            ...
            if job_id is not None:
                ...
                if failed:
                    log_callback(f"Marked job {job_id} as failed.")
                    if _is_paper_out_error(exc):
                        blocked_reprint_source_job_id = job_id
                        printer_recovery_attempt = 0
                        log_callback(f"Paper out blocked printing for {job_id}. Waiting for printer recovery before reprinting it.")
