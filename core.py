...
def check_backend_health(config: AppConfig, timeout: float = 3.0) -> HealthStatus:
    ...
    if not config.access_token.strip():
        return HealthStatus(False, "Connected, access token missing")

    auth_req = request.Request(
        f"{backend_url}/api/events/{parse.quote(config.event_id.strip())}/stations",
        headers={
            "Accept": "application/json",
            "User-Agent": "OrderSystemPrinterService/1.0",
    ...
    try:
        ...
        if 200 <= status_code < 300:
            return HealthStatus(True, "Connected, token valid")
        if status_code == 401:
            return HealthStatus(False, "Connected, token invalid")
        if status_code == 404:
            return HealthStatus(False, "Connected, event not found")
        if status_code == 422:
            return HealthStatus(False, "Connected, Event ID invalid")
